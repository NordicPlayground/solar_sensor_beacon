...
== Firmware
After the initial setup, the firmware will continuously repeat the following
steps:
* Every 10th advertising event and also before the very first advertising
  event:
...
* Reset the radio hardware configuration to operate in BLE DD (Bluetooth
  Low Energy Device Discovery) mode.
